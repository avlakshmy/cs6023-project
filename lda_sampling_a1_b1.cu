...
// nvcc -arch=sm_35 -rdc=true lda_sampling_a1_b1.cu
// ./a.out lda_toi.txt > lda_sampling_a1_b1_outputs.txt
...
__global__ void init_a_kernel(int totWords, int K, double* da) {
  int uid = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char* argv[]) {
  ...
  for (i = K_MIN; i <= K_MAX; i += 8) {
    ...
    // initialising the array to store the relative probabilities
    init_a_kernel<<<numBlocks3, BLOCKSIZE>>>(totWords, K, da);
